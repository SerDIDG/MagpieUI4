    ...
    var requestHandler = function(){
        ...
        if(that.params['showListOnEmpty'] || query.length >= that.params['minLength']){
            that.requestDelay = setTimeout(function(){
                if(that.params['preloadData'] && !cm.isEmpty(that.requestParams['data'])){
                    that.callbacks.data(that, {
                        'data' : that.requestParams['data'],
                        'query' : query
                    });
                }else if(that.isRequest){
                    ...
                    that.requestHandler = that.callbacks.request(that, {
                        'config' : config,
                        'query' : query
                    });
                }else{
